...
def upload_predict():
    if request.method == "POST":
        ...
        def drivers_code(fpath):
                audio_fil=str(fpath)
                signal, sr = librosa.load(audio_fil, res_type='kaiser_fast')
        ...
        brad=drivers_code(fpath=audio_file.filename)

        return render_template('idex.html',prediction=brad)
